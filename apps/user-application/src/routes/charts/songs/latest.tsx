...
export const Route = createFileRoute("/charts/songs/latest")({
  component: LatestSongsPage,
});

function LatestSongsPage() {
  const { data, isLoading } = useQuery({
    queryKey: ["songs", "latest", 20],
    queryFn: async () => {
      const response = await fetch("/api/songs?sortBy=releaseDate&order=desc&limit=20");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 pt-24">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Latest Releases</h1>
          <p className="text-lg text-muted-foreground">
            Top 20 newest songs released
          </p>
        </div>
