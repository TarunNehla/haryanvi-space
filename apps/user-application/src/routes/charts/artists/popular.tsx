...
export const Route = createFileRoute("/charts/artists/popular")({
  component: PopularArtistsPage,
});

function PopularArtistsPage() {
  const { data, isLoading } = useQuery({
    queryKey: ["artists", "popular", 20],
    queryFn: async () => {
      const response = await fetch("/api/artists?sortBy=popularity&order=desc&limit=20");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 pt-24">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Most Popular Artists</h1>
          <p className="text-lg text-muted-foreground">
            Top 20 artists ranked by popularity score
          </p>
        </div>
